...
const PollSelectionDropdown = ({ polls, selectedPoll, onSelect }) => {
  const { t } = I18n;
  ...
  useEffect(() => {
    ...
    for (let pollId in polls) {
      array.push(polls[pollId]);
    }
    setPollList(array);
  }, [polls]);

  const selectLabel = () => {
    if (selectedPoll) return selectedPoll;
    else if (pollList.length === 0) return t('noStoredPolls');
  ...
  const openBottomSheet = () => {
    if (pollList.length > 0) setIsBottomSheetVisible(true);
  };
...
export default PollSelectionDropdown;
